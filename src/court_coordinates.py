...
def make_legacy_output_path(video_path, output_dir):
    """Previous misspelled path, accepted as an input for compatibility."""
    return output_dir / f"{video_path.stem}_coort.json"


def load_existing_annotation(annotation_path, frame_width, frame_height):
    ...
    saved_width = int(data.get("frame_width") or frame_width)
    saved_height = int(data.get("frame_height") or frame_height)
    ...
    for point in scale_initial_keypoints(frame_width, frame_height):
        ...
        if saved_point is not None:
            ...
            point["x"] = None if x is None else int(round(float(x) * scale_x))
            point["y"] = None if y is None else int(round(float(y) * scale_y))
...
def annotate_video(video_path, output_dir, json_court=None):
    ...
    if json_court is not None and not json_court.is_file():
        cap.release()
        raise FileNotFoundError(f"JSON разметки корта не найден: {json_court}")

    legacy_output_path = make_legacy_output_path(video_path, output_dir)
    annotation_path = json_court if json_court is not None else output_path
    if json_court is None and not output_path.exists() and legacy_output_path.exists():
        annotation_path = legacy_output_path
        print(f"Loaded legacy court annotation from {legacy_output_path}")
    keypoints, frame_index = load_existing_annotation(
        annotation_path, frame_width, frame_height
    )
    if json_court is not None:
        print(f"Loaded court annotation from {json_court}")
...
def build_parser():
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Load initial court keypoints from this JSON file",
    )
    return parser
...
def main():
    ...
    output_path = annotate_video(args.video_path, args.output_dir, args.json_court)
    print(f"Saved annotation to {output_path}")
